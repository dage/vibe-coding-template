...
show_usage() {
    ...
    echo ""
    echo "Modes:"
    echo "  single     Run one iteration and exit (default)"
    echo "  loop       Run continuous autonomous iterations"
    echo "  demo       Run demo mode (no API key required)"
    echo "  continue   Resume after human intervention with new direction"
    echo ""
    echo "Examples:"
    echo "  $0                                    # Single iteration"
    echo "  $0 --mode loop                        # Autonomous loop"
    echo "  $0 --mode loop --iterations 5         # Loop with 5 iterations"
    echo "  $0 --mode loop --prompt 'Add dark mode' # Loop with custom direction"
    echo "  $0 --mode demo                        # Demo mode"
